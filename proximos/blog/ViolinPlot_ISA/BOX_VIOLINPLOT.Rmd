...
O violin plot é bastante similar a um box plot, mas com a vantagem de usar um *kernel de densidade simétrico* que se distribui em ambos os lados do "violino"; ou seja, ela também consegue refletir a densidade de probabilidade (como um histograma). A maior do violin plot é que ela consegue ser mais informativo que um boxplot, pois mostra todo o conjunto dos dados. Infelizmente, não é um gráfico tão popular, o que pode levar a alguma confusão sobre sua interpretação.
...
```
(obs: depois que postar o texto no blog, vou fazer um tweet e substituir esse código pelo tweet realizado)
