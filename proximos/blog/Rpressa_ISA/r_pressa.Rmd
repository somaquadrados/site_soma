...
Para vetores, temos:
Observe que em vetores, você pode acessar cada elemento de um vetor chamando justamente a posição dele. O mesmo não é valido quadno você usa o sinal de menos, nesse caso ele remove um elemento do vetor. Um fato interessante sobre vetores é que você pode fazer algumas manipulações em cima de vetores que são mais práticos da vida real. Uma delas com a média (mean(j)). Além disso, observe que multiplicar um vetor v por x (v*x) é equivalente a uma multiplicar cada entrada de v por x.
...
Vamos facilitar a vida de todo mundo. Quanto menos esforço você tiver para conseguir importar e manipular o dados no R de início, melhor vai ser a transição para análise de dados .Isto é, quanto menos você tiver que manipular seus dados no começo, mais  agradável vai ser. Para isso, eu recomendo que você adote algumas boas práticas:
    ...
    * Não tenha mais de informação por célula.
...
Tabela e script na mesma pasta, a gente consegui abrir os dados que a gente baixou da tabela iris no R. Essa tabela contém cinco colunas: sepal.length (comprimento sépalas), sepal.width (largura sépalas), petal.length (comprimento sépalas), petal.width (largura pétalas), species (espécies). Todas essas medidas foram realizadas, como o nome sugere, em diferentes tipos de iris. 
...
Se você chegou aqui, você teve uma primeira experiência em R que eu espero que tenha sido remotamente agradável e que você esteja menos aterroizada de se aventurar em R.